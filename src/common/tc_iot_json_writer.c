...
int tc_iot_json_writer_object_begin(tc_iot_json_writer * w, const char * name) {
    if (name[0] == '{' && name[1] == '\0') {
        return tc_iot_json_writer_raw_data(w, "{", NULL);
    } else {
...
/**
 * @brief    完成写入 json 数组对象。
 *
 * @details  完成写入 json 数组对象，主要是写入： }。
 *
 * @param    w json writer
 *
 * @return   > 0 时，表示处理过程中，往 buffer 区域写入的字节数。 < 0，表示出错
 * @see tc_iot_sys_code_e
 */
int tc_iot_json_writer_object_in_array_end(tc_iot_json_writer * w) {
    int ret = 0;
...
int tc_iot_json_writer_format_data(tc_iot_json_writer * w, const char * name, const char * format, const void * value_ptr) {
    ...
    if (w->buffer[w->pos-1] != '{' && w->buffer[w->pos-1] != '[' ) {
        if (value_ptr) {
            ...
            ret = tc_iot_hal_snprintf(&w->buffer[w->pos], w->buffer_len-w->pos, ",%s",name);
        }
    } else {
        if (value_ptr) {
            ret = tc_iot_hal_snprintf(&w->buffer[w->pos], w->buffer_len-w->pos, "\"");
    ...
    case 's':
        ...
        ret = tc_iot_hal_snprintf(&w->buffer[w->pos], w->buffer_len-w->pos, "%s", (const char *)value_ptr);
